...
<template>
    <Carousel :items-to-show="1">
        <Slide v-for="(slide, index) in sliders" :key="`slide-${index}`">
            <div  :class="`h-[110vh] w-full bg-[15%] bg-cover no-repeat`"
                :style="{
                    backgroundImage: `linear-gradient(180deg, rgba(0, 0, 0, 0.2) 35.37%, rgba(0, 0, 0, 0.5) 61.22%), url('/storage/${slide.image}')`
                }"
            >
                <div class="max-w-7xl mx-auto flex items-end  h-full py-20">
                    <div class="w-9/12 space-y-3">
                        <h1 class="text-[74px] !text-left leading-[80px] hero-title text-white" v-html="format(slide.title)" ></h1>
                        <p class="text-xl w-5/6 text-white !text-left">{{ slide.caption }}</p>
                    </div>
                </div>
            </div>
        </Slide>
