...
function handleNext () {
    step.value = ((step.value + 1) < sliders.length) ? (step.value + 1) : 0;
    next()
}
                    ...
                    <div class="max-w-7xl mx-auto flex items-end  h-full pb-20">
                        ...
                        <div class="w-3/12 flex items-center justify-end space-x-3">
                            <svg class="size-12"  viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <circle cx="18" cy="18" r="16" fill="none" class="stroke-current duration-1000 transition-all text-white" stroke-width="3" stroke-dasharray="100" :stroke-dashoffset="100 - (100 / (step + 1))"></circle>
                                </g>
                            </svg>
        
                            <p class="text-white/50 font-semibold text-3xl tracking-widest">
                                <span class="text-white">{{step + 1}}</span>/{{sliders.length}}
                            </p>
                        </div>
                    </div>
                </div>
